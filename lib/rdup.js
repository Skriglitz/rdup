...
module.exports.createPackageFromFiles = function (src, dest, filenames, metadata, options, callback) {
    ...
    const hashSanitize = function(hash) {
    ...
    if (typeof options.hash !== 'undefined' && options.hash !== null) {
        let sanitized = hashSanitize(options.hash);
        if (sanitized !== null) {
            filesystem.setChecksumType(sanitized);
        }
    }

    if (typeof options.algo !== 'undefined' && options.algo !== null) {
        if (cjs.selectAlgorithm(options.algo) !== null) {
            filesystem.setCompressionType(options.algo);
        }
    }

    if (typeof options.level !== 'undefined' && options.level !== null) {
        if (options.level >= 1 && options.level <= 9) {
            filesystem.setCompressionLevel(options.level);
        }
    }
    ...
    const handleFile = function (filename, done) {
        ...
        switch (file.type) {
            ...
            case 'file':
                ...
                if (options.compressExclude && typeof(options.compressExclude) !== typeof(true)) {
                    shouldCompress = minimatch(filename, options.compressExclude, { matchBase: true });
                }
                files.push({ filename: filename, excluded: shouldExclude, compress: shouldCompress });
                filesystem.insertFile(filename, shouldExclude, shouldCompress, file, options, done);
                return;
            case 'link':
    ...
    return filenames.map((filename) => {
        ...
        if (file.files) {
            ...
            if (!file.excluded) {
                ...
                let sha = crypto.createHash(filesystem.checksumType);
